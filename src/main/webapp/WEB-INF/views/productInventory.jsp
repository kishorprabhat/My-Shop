...
<c:url var="prevUrl"
	value="/admin/productManagement/${currentPageNumber - 1}" />
<c:url var="nextUrl"
	value="/admin/productManagement/${currentPageNumber + 1}" />
	...
	<div class="container">
		...
		<div class="page-header">
			<h1>Product Inventory
				Page:${pageContext.request.userPrincipal.name}</h1>

			<p class="lead">This is product inventory page</p>
			<a class="btn btn-primary"
				href="<spring:url value="/admin/product/addProduct"/>">Add New
				Product</a>

			<c:if test="${not empty search}">
				<a class="btn btn-primary"
					href="<spring:url value="/admin/productManagement/1"/>">See All
					Product</a>
			...
			<div class=row>
				...
				<div class="searchBox">
					...
					<form class="form-inline"
						action="<c:url value="/admin/productManagement/search/1" />"
						method="post">
						<input type="hidden" name="${_csrf.parameterName}"
							value="${_csrf.token}" />
						<div class="form-group">
							<label for="inputEmail">Search more product :</label> <input
								type="text" name="searchTerm" class="form-control"
								style="width: 300px" id="inputEmail"
								placeholder="Enter Brand , model or  category name">
						</div>
